...
export async function POST(request: Request) {
  ...
  if (honeypot) return NextResponse.json({ ok: true, captured: false });

  const startedAt = Number(body.startedAt || 0);
  const elapsed = Date.now() - startedAt;
  if (!Number.isFinite(startedAt) || startedAt <= 0 || elapsed < 1200 || elapsed > 86_400_000) {
    return NextResponse.json({ ok: false, error: "No fue posible validar el formulario." }, { status: 400 });
  }
  ...
  if (!lead.name || !lead.need || !lead.consent) {
    return NextResponse.json(
      { ok: false, error: "Completa nombre, necesidad y autorización de contacto." },
      { status: 422 },
    );
  }

  if (!isEmail(lead.email) && !isPhone(lead.phone)) {
    return NextResponse.json({ ok: false, error: "Agrega un correo o teléfono válido." }, { status: 422 });
  }
  ...
  if (webhookUrl) {
    try {
      ...
      if (!response.ok) console.error("Lead webhook rejected request", response.status);
    } catch (error) {
